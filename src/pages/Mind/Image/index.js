...
class App extends React.Component {
    ...
    render() {
        ...
        return (
            ...
            >
                <Form>
                    <Form.Item label={<span>图片<span style={{ color: '#F4A460' }}>（仅支持jpg、png图片）</span></span>}>
                        {getFieldDecorator('url', { initialValue: nodeInfo.image.url, trigger: '', rules: [{ required: true, message: '请选择图片' }] })(
                            <Upload {...props}>
                                <Button loading={loading} icon='upload' > 点击上传</Button>
                            </Upload>
                        )}
                    </Form.Item>
                </Form>
                {
                    !isUndefined(getFieldValue('url')) && <img className='ui-logo' src={getFieldValue('url')} />
                }
            </Modal>
        );
    }

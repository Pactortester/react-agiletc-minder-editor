...
class App extends React.Component {
    ...
    render() {
        ...
        const loop = data =>
            data.map(item => {
                const text = <span>{isUndefined(item.data.text) ? '[图片]' : item.data.text === '' ? '[无文本]' : item.data.text}</span>
                return <TreeNode key={item.data.id} title={text} />;
            });

        return (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                <Select style={{ margin: '0px 5px' }} value={recordStatus} placeholder='请选择查询的结果状态' onChange={this.handleChange}>
